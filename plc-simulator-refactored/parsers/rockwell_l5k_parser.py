...
class RockwellL5KParser(PLCParser):
    ...
    def _parse_udts(
        ...
        for datatype_section in self._findall("DATATYPE(.*?)END_DATATYPE", l5k_content):
            ...
            # Parse members - handle both \r\n and \n
            members_start = max(datatype_section.find(")\r\n"), datatype_section.find(")\n"))
            if members_start > -1:
                members_start += 2 if datatype_section[members_start+1] == '\r' else 1
                members_start += 1  # Skip the newline
            else:
                members_start = datatype_section.find(")")+1
            members_section = datatype_section[members_start:]
    ...
    def _parse_global_tags(
        ...
        try:
            controller_section = self._findall_one(r"CONTROLLER(.*?)END_TAG(?:\r?\n)", l5k_content)
            tags_section = self._findall_one(r"TAG(?:\r?\n)(.*?)END_TAG(?:\r?\n)", controller_section + "END_TAG\n")
    ...
    def _parse_single_tag(
        ...
        if is_datatype:
            # DATATYPE format: <name> : <type>[<array>] <attributes> OR old format <type> <name>[<array>] <attributes>
