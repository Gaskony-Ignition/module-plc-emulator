...
This guide covers the development workflow, architecture, and procedures for working on the Enhanced PLC Simulator module.
...
Expected output:
```
INFO  [EnhancedSimulatorExtensionPoint] Registering device type: EnhancedSimulator
INFO  [EnhancedSimulatorExtensionPoint] Registered bundle: com.inductiveautomation.plcsimulator...
```
...
```java
// EnhancedSimulatorExtensionPoint.java
public class EnhancedSimulatorExtensionPoint extends AbstractDeviceModuleHook {
    @Override
...
```java
// EnhancedSimulatorDevice.java
public class EnhancedSimulatorDevice extends AbstractEvaluationDriver {
    @Override
...
Device configuration defined in `EnhancedSimulatorConfig.java`:

```java
public class EnhancedSimulatorConfig {
    @FormField(
        ordinal = 0,
...
**File**: `gateway/src/main/resources/.../EnhancedSimulator.properties`
```properties
EnhancedSimulator.Meta.DisplayName=Enhanced PLC Simulator
...
**1. Add enum value** (`EnhancedSimulatorConfig.java`):
```java
...
**2. Add i18n entry** (`EnhancedSimulator.properties`):
```properties
ParserType.MYNEWPARSER.DisplayName=My New Parser (Description)
```

**3. Implement parser logic** (`EnhancedSimulatorDevice.java`):
```java
...
// EnhancedSimulatorDevice.java
protected void connectDevice() {
    EnhancedSimulatorConfig config = getDriverConfig();
...
- Set log level to DEBUG for specific logger:
  - `com.inductiveautomation.plcsimulator`
...
- `EnhancedSimulatorDevice.java:110` - Initialize simulation engine
- `EnhancedSimulatorDevice.java:115` - Setup file watcher for hot reload
- `EnhancedSimulatorDevice.java:146` - Stop simulation engine
- `EnhancedSimulatorDevice.java:309` - Implement parsing for each vendor format
- `EnhancedSimulatorDevice.java:396` - Simulation engine integration
- `EnhancedSimulatorDevice.java:408` - Hot reload support
