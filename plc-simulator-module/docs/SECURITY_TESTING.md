# Security Testing Guide - Enhanced PLC Simulator
...
This guide provides comprehensive security testing procedures for the Enhanced PLC Simulator module. Use this guide to:
...
1. **File Upload Endpoints**
   - `/data/plcsimulator/upload` - REST API for file upload
   - `/res/plcsimulator/edit-program.html` - Web UI for file upload
...
1. **Test Environment**
   ...
   - Enhanced PLC Simulator module v5.4.9 installed
...
**Test**:
```bash
curl -v -X POST http://localhost:8088/data/plcsimulator/upload \
  -H "Content-Type: multipart/form-data" \
  -F "file=@test.L5K" \
  -F "deviceName=TestDevice"
```
...
COOKIE=$(curl -c - http://localhost:8088/data/plcsimulator/upload | grep JSESSIONID)

# Attempt to use pre-auth cookie after victim login
curl -b "$COOKIE" -X POST http://localhost:8088/data/plcsimulator/upload \
  -F "file=@test.L5K"
```
...
(
  echo -n "POST /data/plcsimulator/upload HTTP/1.1\r\n"
...
grep -r "password\s*=" plc-simulator-module/
grep -r "keyPassword" plc-simulator-module/
grep -r "keystorePassword" plc-simulator-module/
```
...
**Location**: `plc-simulator-module/security-test.sh`
...
if grep -r "password\s*=\s*['\"]" plc-simulator-module/; then
...
```
Subject: [SECURITY] Enhanced PLC Simulator - [Vulnerability Type]
...
**Scope**: Enhanced PLC Simulator module v5.x
