...
The Enhanced PLC Simulator transforms PLC export files (L5K, L5X, JSON, CSV) into hierarchical OPC-UA tag structures through an 8-step pipeline:
...
### Starting Point
```
EnhancedSimulatorDevice.onStartup()
Location: gateway/src/main/java/.../gateway/device/EnhancedSimulatorDevice.java:110
```
...
├── device/
│   ├── EnhancedSimulatorDevice.java         # Main device driver (800+ lines)
...
|----------|--------|---------|
| `/data/plcsimulator/upload?device={name}` | POST | Upload PLC file to device |
| `/data/plcsimulator/devices` | GET | List all simulator devices |
| `/data/plcsimulator/device/{name}/status` | GET | Get device status |
| `/data/plcsimulator/device/{name}/delete` | DELETE | Delete uploaded file |
...
├─────────────────────────────────────────────────────────────┤
│ EnhancedSimulatorDevice.onStartup():117                      │
...
**Solution**:
1. Verify `EnhancedSimulatorExtensionPoint` registered in `SimulatorModuleHook.startup()`
...
4. Try REST API directly: `POST /data/plcsimulator/upload?device=DeviceName`
...
3. Check `EnhancedSimulatorConfig.parserType()` returns correct value
...
```bash
curl http://localhost:8088/data/plcsimulator/device/{deviceName}/status
```

#### List All Devices
```bash
curl http://localhost:8088/data/plcsimulator/devices
```
...
curl -X POST \
  ...
  "http://localhost:8088/data/plcsimulator/upload?device=MyDevice"
```
...
```
User/System → POST /data/plcsimulator/upload?device={deviceName}
      ↓
