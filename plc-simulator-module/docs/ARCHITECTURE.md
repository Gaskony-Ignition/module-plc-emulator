# Enhanced PLC Simulator - Architecture Documentation
...
The Enhanced PLC Simulator is an Ignition Gateway module that provides virtual PLC device simulation through OPC-UA. It allows users to upload PLC program files (L5K, L5X, JSON, CSV) which are parsed and exposed as OPC-UA tags in a hierarchical structure.
...
#### EnhancedSimulatorExtensionPoint
...
**Location**: `gateway/device/EnhancedSimulatorExtensionPoint.java`

#### EnhancedSimulatorDevice
...
**Location**: `gateway/device/EnhancedSimulatorDevice.java` (800+ lines)
...
**Location**: `gateway/device/EnhancedSimulatorConfig.java`
...
**Endpoints**:
- `POST /data/plcsimulator/upload?device={name}` - Upload file to device
- `GET /data/plcsimulator/device/{name}/status` - Get device status
- `DELETE /data/plcsimulator/device/{name}/delete` - Delete uploaded file
- `GET /data/plcsimulator/devices` - List all simulator devices
...
**Mounting**: Resources are mounted at `/res/plcsimulator/` via Ignition web framework
...
└──────────────┬──────────────────────────┘
               ↓
┌─────────────────────────────────────────┐
│ EnhancedSimulatorDevice.connect()       │
...
4. Restart Gateway
5. Verify: Config → Devices → Create New Device → Select "Enhanced PLC Simulator"
...
1. **Static Device Registry** (gateway/device/EnhancedSimulatorDevice.java)
