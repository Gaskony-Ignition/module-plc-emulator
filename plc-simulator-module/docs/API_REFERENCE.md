# API Reference - Enhanced PLC Simulator
...
The Enhanced PLC Simulator exposes several APIs for:
...
|----------|-----------|-------------------------|
| **File Upload** | `/data/plcsimulator/*` | Yes (Designer/Admin role) |
| **Web UI** | `/res/plcsimulator/*` | No (public resources) |
...
All `/data/plcsimulator/*` endpoints require authentication using Ignition Gateway credentials.
...
**Example**:
```bash
curl -u admin:password http://localhost:8088/data/plcsimulator/upload
```
...
curl -b cookies.txt -F "file=@test.L5K" \
  http://localhost:8088/data/plcsimulator/upload
```
...
|--------|----------|-------------|---------------|
| `POST` | `/data/plcsimulator/upload` | Upload PLC file and apply to device | Yes |
| `GET` | `/data/plcsimulator/devices` | List all simulator devices | Yes |
| `GET` | `/data/plcsimulator/devices/{name}` | Get device details | Yes |
| `DELETE` | `/data/plcsimulator/devices/{name}` | Delete device | Yes |
| `GET` | `/data/plcsimulator/status` | Get module status | Yes |

---

## File Upload API

### POST /data/plcsimulator/upload
...
fetch('http://localhost:8088/data/plcsimulator/upload', {
  method: 'POST',
  headers: {
...
url = 'http://localhost:8088/data/plcsimulator/upload'
auth = ('admin', 'password')
...
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("http://localhost:8088/data/plcsimulator/upload"))
...
### GET /data/plcsimulator/devices

List all Enhanced PLC Simulator devices.

#### Request

**Method**: `GET`

**URL**: `/data/plcsimulator/devices`
...
**cURL**:
```bash
curl -u admin:password http://localhost:8088/data/plcsimulator/devices
```
...
url = 'http://localhost:8088/data/plcsimulator/devices'
auth = ('admin', 'password')
...
### GET /data/plcsimulator/devices/{name}
...
**URL**: `/data/plcsimulator/devices/{name}`
...
**cURL**:
```bash
curl -u admin:password http://localhost:8088/data/plcsimulator/devices/Building1_PLC
```
...
### DELETE /data/plcsimulator/devices/{name}
...
curl -X DELETE -u admin:password \
  http://localhost:8088/data/plcsimulator/devices/TestDevice
```
...
url = 'http://localhost:8088/data/plcsimulator/devices/TestDevice'
auth = ('admin', 'password')
...
**URL**: `/data/plcsimulator/status`

#### Examples

**cURL**:
```bash
curl -u admin:password http://localhost:8088/data/plcsimulator/status
```
...
<html>
<head>
  <title>Edit Program - Enhanced PLC Simulator</title>
</head>
<body>
  ...
  <script src="/res/plcsimulator/plc-file-upload.js"></script>
</body>
</html>
```

### GET /res/plcsimulator/simple-upload.html

Simple file upload form (alternative to edit-program.html).

**URL**: `http://localhost:8088/res/plcsimulator/simple-upload.html`
...
**Multipart Form Data**:
```http
POST /data/plcsimulator/upload HTTP/1.1
Host: localhost:8088
Authorization: Basic YWRtaW46cGFzc3dvcmQ=
...
class PLCSimulatorClient:
    ...
    def upload_file(self, file_path, device_name, parser_type='ROCKWELL_L5K'):
        ...
        url = f'{self.base_url}/data/plcsimulator/upload'
    ...
    def get_devices(self):
        """List all devices"""
        url = f'{self.base_url}/data/plcsimulator/devices'
        response = requests.get(url, auth=self.auth)
        return response.json()

    def get_device(self, device_name):
        """Get device details"""
        url = f'{self.base_url}/data/plcsimulator/devices/{device_name}'
        response = requests.get(url, auth=self.auth)
        return response.json()
    ...
    def get_status(self):
        """Get module status"""
        url = f'{self.base_url}/data/plcsimulator/status'
        response = requests.get(url, auth=self.auth)
        return response.json()
...
class PLCSimulatorAPI {
  ...
  async uploadFile(file, deviceName, parserType = 'ROCKWELL_L5K') {
    ...
    try {
      const response = await fetch(`${this.baseURL}/data/plcsimulator/upload`, {
        method: 'POST',
        headers: {
  ...
  async getDevices() {
    const response = await fetch(`${this.baseURL}/data/plcsimulator/devices`, {
      headers: {
        'Authorization': `Basic ${this.auth}`
      }
    });
...
for file in "${!files[@]}"; do
  ...
  response=$(curl -s -w "\n%{http_code}" -u "$USERNAME:$PASSWORD" \
    ...
    -F "deviceName=$device" \
    -F "parserType=ROCKWELL_L5K" \
    "$BASE_URL/data/plcsimulator/upload")
...
public class PLCSimulatorClient {
    ...
    public UploadResult uploadFile(Path filePath, String deviceName, String parserType)
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create(baseURL + "/data/plcsimulator/upload"))
    ...
    public DeviceList getDevices() throws IOException, InterruptedException {
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create(baseURL + "/data/plcsimulator/devices"))
            .header("Authorization", authHeader)
            .GET()
            .build();
