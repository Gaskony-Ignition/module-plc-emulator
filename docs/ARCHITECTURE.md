...
The Logix PLC Emulator is an Ignition Gateway module that provides virtual PLC device simulation through OPC-UA. It allows users to upload PLC program files (L5K, JSON, CSV) which are parsed and exposed as OPC-UA tags in a hierarchical structure.

### Key Features
- **Rockwell Logix PLC file support** (L5K, JSON, CSV)
...
- **Parser Type**: Rockwell (L5K), JSON, CSV
...
#### L5XParser
**Purpose**: Parses Rockwell Studio 5000 XML export format (.l5x). **Not a supported
user-facing format since 11.0.0** — unregistered from `ParserFactory` and unreachable from
any upload; retained only as the reference implementation `L5KParser` is held to by
`AoiCrossFormatEquivalenceTest`. See `PROJECT_CHARTER.md` §4.
...
1. Check file extension (.l5k, .json, .csv)
2. If unknown, attempt content-based detection
3. `.l5x` resolves to no parser at all (format withdrawn in 11.0.0)
...
- **L5X Format**: Rejected outright with guidance to re-export as L5K (withdrawn in 11.0.0)
...
│ - Format validation (L5K/JSON/CSV)      │
