...
- Uploading PLC files (L5K, JSON, CSV)
- Managing devices
...
> **HTTP Basic Authentication does NOT work against this module.**
...
|--------|----------|--------------|-------------|
| `X-Filename` | No | Original filename, used for format auto-detection (`.l5k`, `.json`, `.csv`) | Sanitised via `PathSecurity.sanitizeFileName()`; defaults to `uploaded_file.txt` if absent |
...
**Error (422 Unprocessable Entity)** — file saved, but the device failed to apply it:
```json
{
  "success": false,
  "filename": "tags.json",
...
{
  "success": false,
  "error": "L5X (Studio 5000 XML export) is not a supported format. Export your program as an L5K file from Studio 5000 and upload that instead. Supported formats: .l5k, .json, .csv"
}
```
...
- **Allowed Extensions**: `.L5K`, `.JSON`, `.CSV` only (`.L5X` is rejected with guidance to re-export as L5K — withdrawn in 11.0.0)
...
`filename` must be one of the `filename` values returned by `GET /device/:name/versions` —
it is matched against that same listing server-side instead of being built into a file path
directly, so an arbitrary or path-traversal filename cannot be restored.
...
<body>
  ...
  <div id="drop-zone">
    Drag and drop your L5K, JSON, or CSV file here
  </div>
