...
### 3.10a Initial values — scope note

This document does not otherwise specify initial-value derivation (it is a NodeId/
addressing grammar, not a value semantics one), but the scope boundary is recorded
here so it lives where the rest of the addressing decisions do.

- `L5XParser.extractValue()` reads only the top-level scalar `<DataValue Value="...">`
  for an atomic tag. A structure's initial values — `<DataValueMember>` elements nested
  inside a `<Structure>` (e.g. a `TIMER` instance's `.PRE`) — are NOT read; every such
  member gets its type-default value regardless of what the export actually contains (a
  real export with `TIMER.PRE=5000` starts the emulator's copy at `0`). This is a
  deliberate scope boundary, not an oversight — see `KNOWN_ISSUES.md`.
- **Top-level array tags are the exception.** `L5XParser.extractArrayElementValues()`
  reads a top-level array's decorated `<Array><Element Index="..." Value="..."/></Array>`
  ...
  `AddressPolicy.boolArrayBit`. **Still deferred:** per-member values of a
...
## 4. Test-assertion checklist (`@Tag("fidelity")`)
...
### 4.1 Controller scope — `CompactLogix5370-1769L33ER-fw33-NodeblueAI.L5X`
...
### 4.2 Program scope — same file, `MainProgram` / `MotorProgram`
...
### 4.3 UDT member depth + External Access — same file (AOI `Motor_Control`)
...
### 4.4 1-D and multi-dim arrays — `CompactLogix5370-1769L33ER-fw30-stellentus.L5X`
...
### 4.5 3-D array + multi-dim (large) — `ControlLogix-1756L83E-fw36-L5Sharp.L5X`
...
### 4.6 STRING — `ControlLogix-1756L83E-fw36-L5Sharp.L5X`
...
### 4.7 BOOL array (DWORD packing) — synthetic fixture `boolpack.l5x`
...
### 4.8 Bit of a DINT — `ControlLogix-1756L85E-fw38-RockwellAutomation.L5X`
...
### 4.9 Predefined member correctness — synthetic `predefined.l5x` + corpus
...
### 4.10 I/O modules — `ControlLogix-1756L72-fw37-iotrustlab-controller.L5X`
...
### 5.1 DOC-CONFIRMED findings that VALIDATE or SHARPEN the gap analysis
...
### 5.2 Where the gap analysis was INCOMPLETE / needed correction
...
### 5.3 INFERRED rules — the list a real IA bench test must confirm
