...
**Task:** Phase 1 of `docs/plans/V10_1_L5K_PLAN.md`. This is the single normative
document the Phase-2 `L5KParser` rewrite and its `@Tag("l5k")` tests are written
against. It defines the exact block grammar, tag-line grammar, and semantic mapping the
parser must implement so that a Studio 5000 **`.L5K`** export produces the same
vendor-neutral parsed-tag model an equivalent `.L5X` export does, feeding
`ADDRESSING.md` unchanged.

**Status:** DRAFT for lead review. Do NOT commit until reviewed.
**Author:** Phase-1 Opus grammar agent, 2026-07-12.
...
- `docs/plans/ADDRESSING.md` — NodeId / expansion / ExternalAccess contract this parser feeds.
