...
testing of the address-space builder. All files below are licence-clean
(MIT or Apache-2.0) and retrieved from public GitHub repositories at a
pinned commit SHA (permalinks, not moving branch heads). Full provenance
detail (construct coverage checklists, exclusions considered) is preserved
in `~/Downloads/plc-v10-artifacts/plc-fidelity/corpus/manifest.md`.
...
- The L5K file is vendored for future L5K work per the corpus's construct
  coverage (it is the only L5K sample) but is not required to have tests in
  Stage A — L5X is the primary/fully-verified Rockwell format (see
  `docs/plans/V10_FIDELITY_PLAN.md`, maintainer decision 10/07/2026).
