...
public class FileValidator {
    ...
    // L5K is the only supported Rockwell format (maintainer decision 27/07/2026, see
    // CHANGELOG "Removed"): L5X is rejected explicitly by isRejectedL5x() below rather
    // than falling through to the generic "unsupported format" message, so the operator
    // gets an actionable instruction instead of a dead end.
    private static final List<String> SUPPORTED_EXTENSIONS = Arrays.asList(
        ".l5k", ".json", ".csv"
    );

    /** Guidance returned for a deliberately-unsupported L5X upload. */
    private static final String L5X_UNSUPPORTED_MESSAGE =
        "L5X (Studio 5000 XML export) is not a supported format. Export your program as an "
        + "L5K file from Studio 5000 and upload that instead. Supported formats: "
        + ".l5k, .json, .csv";
    ...
    public static ValidationResult validateFile(File file, long maxSizeMB) {
        ...
        if (fileName.endsWith(".l5x")) {
            return ValidationResult.failure(L5X_UNSUPPORTED_MESSAGE);
        }
