...
/**
 * Extension point for the Logix PLC Emulator device.
...
public class LogixEmulatorExtensionPoint extends DeviceExtensionPoint<LogixEmulatorConfig> {
     ...
     */
    public static final String TYPE_ID = "com.gaskony.logixemulator.LogixEmulator";

    /**
     * Constructor registers this device type with Ignition.
     * IMPORTANT: DeviceExtensionPoint expects i18n KEYS, not direct strings!
     * These keys must exist in LogixEmulator.properties and the bundle must be registered in ModuleHook.
     */
    public LogixEmulatorExtensionPoint() {
        super(
            TYPE_ID,
            "LogixEmulator.Meta.DisplayName",      // i18n key (NOT direct text!)
            "LogixEmulator.Meta.Description",      // i18n key (NOT direct text!)
            LogixEmulatorConfig.class
        );
    }
    ...
    protected Device createDevice(
        ...
        LogixEmulatorConfig deviceConfig) {

        return new LogixEmulatorDevice(context, deviceConfig);
    }
    ...
    public Optional<WebUiComponent> getWebUiComponent(ComponentType type) {
        return Optional.of(
            new ExtensionPointResourceForm(
                ...
                SchemaUtil.fromType(LogixEmulatorConfig.class),
                Set.of("/res/logixemulator/plc-file-upload.js")
            )
        );
    }
     ...
     */
    @Override
    protected void validate(LogixEmulatorConfig config, Builder errors) {
