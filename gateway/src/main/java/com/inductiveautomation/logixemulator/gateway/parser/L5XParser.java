...
/**
 * Parser for Rockwell L5X files (Studio 5000 / RSLogix 5000 XML export).
 *
 * <p><strong>NOT A SUPPORTED USER-FACING FORMAT (since 11.0.0).</strong> L5K is the only
 * supported Rockwell format; {@code FileValidator} rejects {@code .l5x} uploads outright
 * and {@code FilePreparation} does not pick {@code .l5x} files off disk, so nothing in
 * the running module reaches this class. It is retained deliberately, not by oversight:
 * it is the reference implementation {@code L5KParser} is held to by
 * {@code AoiCrossFormatEquivalenceTest} (the swap-fidelity contract), and it backs the
 * shared address-space coverage in {@code AddressSpaceBuilderIntegrationTest} against the
 * real-world corpus. Deleting it would delete both. Do not re-expose it through the
 * upload path without a charter amendment — see PROJECT_CHARTER.md §4.</p>
 *
 * <p>Parses the XML structure to extract controller information, controller- and
 * program-scoped tags, User Defined Types (UDTs), and arrays/complex structures.</p>
 */
