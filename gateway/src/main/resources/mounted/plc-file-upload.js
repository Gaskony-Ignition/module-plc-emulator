/**
 * Enhanced Logix Emulator - File Upload Enhancement
 *
 * This script adds a file upload button to the device configuration form.
 * Auto-loaded when viewing Enhanced Logix Emulator device config pages.
 *
...
(function() {
    ...
    console.log('Logix Emulator: File upload enhancement loading...');

    // Prevent multiple injections
    if (window.logixEmulatorUploadInjected) {
        console.log('Logix Emulator: Upload UI already injected');
        return;
    }
    window.logixEmulatorUploadInjected = true;
    ...
    function initFileUpload() {
        ...
        if (!textarea) {
            console.log('Logix Emulator: File content textarea not found, retrying in 1s...');
        ...
        if (textarea.parentElement.querySelector('.plc-upload-button')) {
            console.log('Logix Emulator: Upload button already exists');
            return;
        }

        // Create file upload UI
        createUploadUI(textarea);
        console.log('Logix Emulator: File upload UI injected successfully');
    }
    ...
    function createUploadUI(textarea) {
        ...
        fileInput.type = 'file';
        // Must mirror FileValidator.SUPPORTED_EXTENSIONS. '.xml' was previously offered
        // here but the server has never accepted it, so picking one always failed.
        fileInput.accept = '.l5k,.L5K,.json,.JSON,.csv,.CSV';
        ...
        infoText.innerHTML = '<strong>Upload a file</strong> or paste content below. Supported: L5K, JSON, CSV';
        ...
        fileInput.addEventListener('change', function(e) {
            ...
            reader.onload = function(event) {
                ...
                console.log('Logix Emulator: File loaded successfully -', file.name);
            };
    ...
    console.log('Logix Emulator: File upload enhancement loaded successfully');
})();
